...
export class InsertBuilder<T> extends CrudBaseBuilder<T, InsertColumnsBuilder<T>>{
    ...
    public columns(columnsCallback: (columns: InsertColumnsBuilder<T>) => void): InsertBuilder<T> {
        return super.columnsBase(columnsCallback, new InsertColumnsBuilder<T>(this._metadata, this._modelToSave), this);
    }
    ...
    protected buildBase(): CrudCompiled {
        ...
        return {
            sql: `INSERT INTO ${this._tablename} (${this.getColumnsCompiled().columns.join(", ")}) VALUES (${parameterValues.join(", ")})`,
